...
function toBnNum(numStr) {
  return String(numStr).split('').map(c => enToBnNums[c] || c).join('');
}
...
async function run() {
  ...
  const existingFiles = await readdir(studentsDir);
  const existingMap = {};
  for (const f of existingFiles) {
    if (f.endsWith('.json')) {
      const data = JSON.parse(await readFile(path.join(studentsDir, f), 'utf-8'));
      existingMap[data.student_id] = data;
    }
  }

  // Generate Students
  for (let index = 0; index < studentSeeds.length; index++) {
    const [slug, name, bnName] = studentSeeds[index];
    if (existingMap[slug]) continue; // keep existing manual edits
    ...
    const address = overrides.address || ["Kaliganj, Dhaka", "Narayanganj", "Cumilla", "Gazipur", "Mymensingh", "Sylhet", "Barishal", "Tangail"][index % 8];
    const addressBn = overrides.address || ["কালীগঞ্জ, ঢাকা", "নারায়ণগঞ্জ", "কুমিল্লা", "গাজীপুর", "ময়মনসিংহ", "সিলেট", "বরিশাল", "টাঙ্গাইল"][index % 8];

    const studentData = {
      position: index + 1,
      student_id: slug,
      name_en: overrides.name || name,
      name_bn: overrides.bnName || bnName,
      room_no: room,
      roommate_ids: [],
      short_roll: roll,
      full_roll: fullRoll,
      class_no: "11",
      group: group,
      group_en: group === 'science' ? 'Science' : group === 'arts' ? 'Humanities' : 'Business Studies',
      group_bn: group === 'science' ? 'বিজ্ঞান' : group === 'arts' ? 'মানবিক' : 'ব্যবসায় শিক্ষা',
      section: section,
      practical_group: practicalGroup,
      blood_group: overrides.blood || bloodGroups[index % bloodGroups.length],
      address_en: address,
      address_bn: addressBn,
      phone: phone,
      father_phone: overrides.fatherPhone || `0180000${String(index + 1).padStart(4, "0")}`,
      email: overrides.email || `${slug}@gmail.com`,
      facebook: overrides.fb || "https://facebook.com/",
      bio_en: overrides.bio || "Demo student profile for International Hall. Detailed information can be updated later from the student sheet.",
      bio_bn: overrides.bio || "আন্তর্জাতিক ছাত্রাবাসের ডেমো শিক্ষার্থী প্রোফাইল। পরে sheet থেকে বিস্তারিত তথ্য আপডেট করা যাবে।",
      photo: `images/${slug}.jpg`,
      pdfs: {
        ct1: `pdfs/${slug}-11-ct1.pdf`,
        ct2: `pdfs/${slug}-11-ct2.pdf`,
        hy: `pdfs/${slug}-11-hy.pdf`,
        ct3: `pdfs/${slug}-11-ct3.pdf`,
        yearly: `pdfs/${slug}-11-y.pdf`
      },
      custom_links: []
    };

    await writeFile(path.join(studentsDir, `${slug}.json`), JSON.stringify(studentData, null, 2), "utf8");
    console.log(`Generated student: ${slug}`);
  }

  // Generate Rooms
  for (let i = 1; i <= 14; i++) {
    const roomNo = String(100 + i);
    const roomFile = path.join(roomsDir, `${roomNo}.json`);
    
    // Check if room already exists to not overwrite it
    try {
      await readFile(roomFile);
      continue;
    } catch(e) {} // doesn't exist, generate

    const roomData = {
      room_no: roomNo,
      title_en: `Room ${roomNo}`,
      title_bn: `রুম ${toBnNum(roomNo)}`,
    ...
    await writeFile(roomFile, JSON.stringify(roomData, null, 2), "utf8");
    console.log(`Generated room: ${roomNo}`);
  }
}
